...
export default function ImageUploader({
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      // Ignore URL.createObjectURL failure
    }
      ...
      {activeValue ? (
        <div className="border border-[#1A1512]/20 bg-[#F0EBE1] p-3 space-y-3">
          <div className="relative aspect-[16/9] w-full bg-[#1A1512]/5 overflow-hidden border border-[#1A1512]/15 group">
            <img
              src={localPreview || resolveImageUrl(activeValue)}
              alt="Uploaded high-quality preview"
              className="w-full h-full object-cover"
              onError={(e) => {
                if (localPreview && e.currentTarget.src !== localPreview) {
                  e.currentTarget.src = localPreview;
                } else if (activeValue && activeValue.startsWith('/api/')) {
                  const fallback = apiUrl(activeValue);
