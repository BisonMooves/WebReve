    ...
    if (isConnected && db) {
      ...
      for (const inq of existingInquiries) {
        ...
        if (!existingConv) {
          ...
          const convDoc = formatInquiryDoc({
            id: convId,
            reference: refCode,
            ...
            status: inq.status?.toLowerCase() === 'new' ? InquiryStatus.NEW : (inq.status?.toLowerCase() || InquiryStatus.NEW),
            unreadCount: 1,
          ...
          const msgDoc = formatMessageDoc({
            conversationId: convId,
            type: MessageType.INBOUND,
            channel: MessageChannel.FORM,
...
async function initCollectionsAndIndexes() {
  try {
    if (isConnected && db) {
      ...
      // Clean up any default template sample projects from database
      const DEFAULT_SAMPLE_IDS = ['lumina-os', 'aether-fashion', 'nexus-capital', 'chronos-watch', 'verve-health', 'solaria-living'];
      const deletedSampleCount = await db.collection('projects').deleteMany({ id: { $in: DEFAULT_SAMPLE_IDS } });
      if (deletedSampleCount.deletedCount > 0) {
        console.log(`🧹 Purged ${deletedSampleCount.deletedCount} default template projects from MongoDB Atlas.`);
      }
      ...
      for (const email of ALLOWED_ADMIN_EMAILS) {
        if (!localAdminUsers.has(email)) {
          const defaultPasswordHash = await bcrypt.hash('password123', 10);
          localAdminUsers.set(email, formatAdminDoc({
            email,
            name: email.split('@')[0],
            role: 'admin',
            passwordHash: defaultPasswordHash
...
apiRouter.post('/inquiries', async (req, res) => {
  try {
    ...
    const conversationDoc = formatInquiryDoc({
      id: conversationId,
    ...
    const messageDoc = formatMessageDoc({
      conversationId,
      type: MessageType.INBOUND,
      channel: MessageChannel.FORM,
      body: message || 'New contact inquiry submitted.',
      createdAt: nowIso
    });
    ...
    if (isConnected && db) {
      ...
      localMessages.set(messageDoc.id, messageDoc);
    }
...
apiRouter.post('/admin/conversations/:id/messages', authenticateAdmin, async (req, res) => {
  try {
    ...
    const nowIso = new Date().toISOString();
    const newMsg = formatMessageDoc({
      conversationId: id,
      type: type || MessageType.OUTBOUND,
      channel: channel || MessageChannel.EMAIL,
      body: body.trim(),
      subject: subject ? subject.trim() : null,
      createdAt: nowIso,
      readAt: type === 'inbound' ? null : nowIso
    });
    ...
    if (isConnected && db) {
      ...
      localMessages.set(newMsg.id, newMsg);
...
const handleCreateProject = async (req, res) => {
  try {
    ...
    const newProject = formatProjectDoc(projectData);
    ...
    if (isConnected && db) {
      ...
      console.log(`✨ Saved project to MongoDB: ${newProject.title} (${newProject.id})`);
    } else {
      console.warn(`⚠️ Cannot save project to MongoDB (isConnected: ${isConnected}, db: ${!!db})`);
    }
    localProjects.set(newProject.id, newProject);
...
const handleUpdateProject = async (req, res) => {
  try {
    ...
    let existing = null;
    if (isConnected && db) {
      existing = await db.collection('projects').findOne({ id });
    } else {
      existing = localProjects.get(id);
    }
    const updatedProject = formatProjectDoc({ ...updatedFields, id }, existing);
    ...
    localProjects.set(id, updatedProject);

    res.json({ success: true, message: 'Project updated successfully.', project: updatedProject, savedToMongo: isConnected && !!db });
  } catch (err) {
  ...
  try {
    ...
    if (isConnected && db) {
      for (const p of incomingProjects) {
        if (!p.id) continue;
        const formattedProject = formatProjectDoc(p);

        await db.collection('projects').updateOne(
          { id: formattedProject.id },
          { $set: formattedProject },
          { upsert: true }
        );
        localProjects.set(formattedProject.id, formattedProject);
      }
      ...
      for (const p of incomingProjects) {
        if (!p.id) continue;
        localProjects.set(p.id, formatProjectDoc(p));
      }
...
if (fs.existsSync(distPath)) {
  ...
  app.use((req, res, next) => {
    ...
    return res.sendFile(indexPath, (err) => {
      if (err) {
        res.status(404).send('WebRêve: Frontend bundle not found.');
      }
    });
  });
}
